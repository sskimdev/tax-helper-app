...
export default function FilingRequestDetailPage(): React.ReactNode {
  ...
  const fetchRequestDetail = useCallback(async (): Promise<void> => {
    if (!requestId || !user) {
      setActionError("잘못된 접근이거나 로그인이 필요합니다.");
    ...
    try {
      const { data, error: dbError } = await supabase
        .from('filing_requests').select<'*', FilingRequest>('*').eq('id', requestId).eq('user_id', user.id).single();
  ...
  const handleConfirmCancel = async (): Promise<void> => {
    if (!request?.id || !user?.id) { toast({ title: "오류", description: "취소할 의뢰 정보 없음", variant: "destructive" }); return; }
    if (request.status !== 'submitted') { toast({ title: "알림", description: "이미 처리 중이거나 완료/취소된 의뢰는 취소 불가", variant: "default" }); return; }
  ...
  const handleFileDownload = async (fileToDownload: AttachedFile): Promise<void> => {
    if (!fileToDownload || !fileToDownload.path) { toast({ title: "오류", description: "다운로드할 파일 정보가 유효하지 않습니다.", variant: "destructive"}); return; }
    toast({ title: "정보", description: `${fileToDownload.name} 다운로드를 준비 중입니다...`, duration: 2000 });
    try {
      const { data, error } = await supabase.storage.from(FILING_ATTACHMENTS_BUCKET).createSignedUrl(fileToDownload.path, 3600); // 1시간 유효
  ...
  const handleAddNewFiles = async (): Promise<void> => {
    ...
    try {
      ...
      for (const file of stagedFilesForNewUpload) {
        const fileNameWithTimestamp = `<span class="math-inline">\{Date\.now\(\)\}\_</span>{file.name.replace(/\s+/g, '_')}`;
        const filePath = `<span class="math-inline">\{user\.id\}/</span>{requestId}/${fileNameWithTimestamp}`; // 의뢰ID 폴더 하위에 저장
      ...
      if (uploadFailed) return; // 업로드 중 하나라도 실패하면 DB 업데이트 안함
      ...
      if (newlyUploadedFileInfos.length > 0) {
        ...
        setStagedFilesForNewUpload([]); setShowFileUpload(false); setClearFileUploadSignal(true); // FileUpload 초기화
      }
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto p-4 md:p-6">
      <div className="flex justify-between items-center">
           <h1 className="text-3xl font-semibold">의뢰 상세 정보</h1>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="pt-6 border-t mt-6">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold flex items-center">
                    <Paperclip className="mr-2 h-5 w-5 text-muted-foreground" /> 첨부 파일
                </h3>
                {(request.status === 'submitted' || request.status === 'assigned') && !showFileUpload && (
                    <Button variant="outline" size="sm" onClick={() => {setShowFileUpload(true); setClearFileUploadSignal(false);}}>
                        <PlusCircle className="mr-2 h-4 w-4"/> 파일 추가
                    </Button>
                )}
            </div>
            ...
            {showFileUpload && (
              <Card className="mb-4 p-4 border-dashed bg-muted/20">
                ...
                <CardContent className="p-0">
                  <FileUpload
                    onFilesStaged={handleNewFilesStaged}
                  ...
                  <div className="mt-4 flex justify-end space-x-2">
                    ...
                    <Button size="sm" onClick={handleAddNewFiles} disabled={stagedFilesForNewUpload.length === 0 || isUploadingNewFiles}>
            ...
            {(!request.attached_files || request.attached_files.length === 0) ? (
              <p className="text-sm text-muted-foreground py-4 text-center">첨부된 파일이 없습니다.</p>
            ) : (
              <ul className="space-y-2">
                {request.attached_files.map((file) => (
                  <li key={file.path} className="flex items-center justify-between p-3 border rounded-md hover:bg-muted/50 transition-colors">
                    <div className="flex items-center space-x-3 truncate flex-1 min-w-0">
                      ...
                      <div className="truncate min-w-0">
                        <button type="button" onClick={() => handleFileDownload(file)} className="text-sm font-medium text-primary hover:underline cursor-pointer block text-left truncate" title={file.name}>{file.name}</button>
                        <span className="text-xs text-muted-foreground">({(file.size / 1024).toFixed(1)} KB) - {formatDate(file.uploaded_at)}</span>
                      </div>
                    </div>
                    <div className="flex items-center space-x-1 ml-2 flex-shrink-0">
                      <Button variant="outline" size="sm" onClick={() => handleFileDownload(file)} title="다운로드">
                        <Download className="mr-1.5 h-4 w-4" /> 다운로드
                      </Button>
                      {file.path.startsWith(`${user?.id}/`) && request.status === 'submitted' && (
                         <AlertDialog>
                             <AlertDialogTrigger asChild>
                                <Button variant="ghost" size="icon" title="삭제" className="text-destructive hover:text-destructive hover:bg-destructive/10">
                                    <Trash2 className="h-4 w-4" />
                                </Button>
                             </AlertDialogTrigger>
                             <AlertDialogContent>
                                <AlertDialogHeader><AlertDialogTitle>파일 삭제 확인</AlertDialogTitle><AlertDialogDescription>정말로 '{file.name}' 파일을 삭제하시겠습니까?</AlertDialogDescription></AlertDialogHeader>
                                <AlertDialogFooter><AlertDialogCancel>취소</AlertDialogCancel><AlertDialogAction onClick={() => handleFileDelete(file)} className="bg-destructive hover:bg-destructive/90">삭제</AlertDialogAction></AlertDialogFooter>
                             </AlertDialogContent>
                         </AlertDialog>
                      )}
                    </div>
          ...
          {request.status === 'submitted' && ( <div className="pt-6 border-t mt-6 flex flex-wrap gap-2"> <Link to={`/my-filings/${request.id}/edit`}> <Button variant="outline" disabled={isCancelling}>의뢰 수정하기</Button> </Link> <AlertDialog> <AlertDialogTrigger asChild> <Button variant="destructive" disabled={isCancelling}> {isCancelling ? '취소 처리 중...' : '의뢰 취소하기'} </Button> </AlertDialogTrigger> <AlertDialogContent> <AlertDialogHeader><AlertDialogTitle>의뢰 취소 확인</AlertDialogTitle><AlertDialogDescription>정말로 이 신고 의뢰를 취소하시겠습니까?</AlertDialogDescription></AlertDialogHeader> <AlertDialogFooter><AlertDialogCancel disabled={isCancelling}>닫기</AlertDialogCancel><AlertDialogAction onClick={handleConfirmCancel} disabled={isCancelling}>확인 (취소)</AlertDialogAction></AlertDialogFooter> </AlertDialogContent> </AlertDialog> </div> )}
          {request.status === 'cancelled' && ( <p className="text-sm text-destructive pt-4 border-t">이 의뢰는 사용자에 의해 취소되었습니다.</p> )}
          {['assigned', 'processing', 'completed'].includes(request.status) && ( <p className="text-sm text-muted-foreground pt-4 border-t">이미 전문가에게 배정되었거나 처리가 시작/완료된 의뢰는 수정하거나 취소할 수 없습니다.</p> )}
        </CardContent>
      </Card>
    </div>
  );
